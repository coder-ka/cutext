  ...
  compile(src) {
    ...
    while (src !== '') {
      ...
      for (const cutter of this.cutters) {
        ...
        while (!once && (result = cutter.cut(src, this))) {
  ...
  render(src) {
    ...
    while (src !== '') {
      ...
      for (const cutter of this.cutters) {
        ...
        while (!once && (result = cutter.cut(src, this))) {
          if (noPatternMatchedString !== '') {
            rendered += this.noPattermMatchedCutter.render.call(this, noPatternMatchedString);
          }
        ...
        if (matched) {
          rendered += cutter.render.call(this, cutter.multiple ? tokens : tokens[0]);

          if (unmatchedString) {
            rendered += this.noPattermMatchedCutter.render.call(this, unmatchedString);
          }
    ...
    if (noPatternMatchedString !== '') {
      rendered += this.noPattermMatchedCutter.render(noPatternMatchedString);
    }
